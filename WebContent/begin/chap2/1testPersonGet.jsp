...
<script type="text/javascript">
	...
	function goSubmit() {
		...
		frm.method = "GET";
		frm.action = "<%= request.getContextPath() %>/1testPersonGet.do"; 
		frm.submit();
	}
	<%-- <%= request.getContextPath() %>는 컨텍스트명을 알려주는 것 --> 결과: '/MyWeb'이 나옴 
	!!!중요 암기!!!
	웹경로를 나타낼때 맨 앞에나오는 /는 확장자가 .html또는 .jsp일 경우에는 
	ip주소:포트번호가 생략된 것이고
	.java 또는 .xml일 경우에는
	ip주소:포트번호/컨텍스트명이 생략된 것이다.
	
	ex) html,jsp : /MyWeb/begin/chap2/1testPersonGet.do
	==> 127.0.0.1:9090 /MyWeb/begin/chap2/1testPersonGet.do
	java,xml : /begin/chap2/1testPersonGet.do
	==> 127.0.0.1:9090 /MyWeb/begin/chap2/1testPersonGet.do
	--%>
